  ...
  this._consumer(consumerKey, function(err, consumer, consumerSecret) {
    ...
    function validate(tokenSecret, ok) {
      var url = utils.originalURL(req, self._host)
        , query = req.query
        , body = req.body;
      console.log('middleware validate', url, query, body)
      ...
      var normalizedURL = utils.normalizeURI(url)
        ...
        console.log('normalizedURL', normalizedURL, normalizedParams, req.method)

      if (signatureMethod == 'RSA-SHA1') {
        console.log("signatureMethod RSA-SHA1 ", signature, base)
        ...
        console.log("computedSignature HMAC-SHA1 ", computedSignature, signature)
