...
ConsumerStrategy.prototype.authenticate = function(req) {
  ...
  var consumerKey = params['oauth_consumer_key']
    ...
    , tokenSecretFromParams = params['oauth_token_secret']
  ...
  if (version && version !== '1.0' && !this._ignoreVersion) {
    console.log("fail cause version not set")
  ...
  this._consumer(consumerKey, function(err, consumer, consumerSecret) {
    if (err) { 
      console.log("fail 4 _consumer", err)
      return self.error(err);
    }
    if (!consumer) {
      console.log("fail 5 consumer_key_rejected")
    ...
    if (!requestToken) {
      ...
      validate('', function() {
        ...
        //          users.
        console.log('attempting to obtain a request token ', info)
      ...
      self._token(requestToken, function(err, tokenSecret, info) {
        if (err) {
          console.log("fail 6 requestToken", err)
          return self.error(err);
        }
        if (!tokenSecret) tokenSecret = tokenSecretFromParams;
        if (!tokenSecret) { 
          console.log("fail 7 token_rejected", tokenSecret, info)
          return self.fail(self._challenge('token_rejected'));
        }
    ...
    function validate(tokenSecret, ok) {
      ...
      if (signatureMethod == 'RSA-SHA1') {
        var verified = utils.verify_rsasha1(self._publicKey, signature, base);
        ...
        if (signature !== computedSignature) {
          console.log("invalid computedSignature ", computedSignature)
          return self.fail(self._challenge('signature_invalid'));
        }
        ...
        console.log("signature_method_rejected")
